...
class VerificationStatus(Enum):
    ...
    FOUND = "found"           # Real test case matches ideal verification
    NOT_FOUND = "not_found"   # No existing test can verify this
    PARTIAL = "partial"       # Test exists but doesn't fully cover verification need
...
class ModuleSummary:
    ...
    module_id: int
    module_title: str
    summary: str                                    # 2-line summary of what the module does
    verification_keywords: List[str] = field(default_factory=list)  # Keywords for matching
    can_verify_states: List[str] = field(default_factory=list)      # States this module can verify
    action_states: List[str] = field(default_factory=list)          # States this module modifies
...
class TestCase:
    ...
    needs_post_verification: bool = False           # Whether this test needs verification
    modifies_state: List[str] = field(default_factory=list)  # States this test modifies
    post_verifications: List[Dict] = field(default_factory=list)  # Verification details
    verification_coverage: str = ""                 # "full" | "partial" | "none" | ""
    coverage_gaps: List[str] = field(default_factory=list)  # What can't be verified
    ...
    def to_dict(self) -> dict:
        ...
        # Include post-verification fields if test needs verification
...
class IdealVerification:
    ...
    description: str              # "Check source account balance decreased"
    target_module: str            # "Account Overview"
...
class VerificationMatch:
    ...
    ideal_description: str        # From IdealVerification
    status: str                   # "found" | "not_found" | "partial"
    matched_test_id: str = ""     # Test ID if found
    matched_test_title: str = ""  # Test title if found
    confidence: float = 0.0       # RAG similarity score
